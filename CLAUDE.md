...
Context for Claude Code sessions on this repo. Read this before doing anything; the full task breakdown lives in `PLAN.md`.
...
```
.claude/agents/   custom subagents — invoke explicitly with @agent-<name>
...
Four custom agents live in `.claude/agents/`. Invoke them explicitly — auto-routing is unreliable.
